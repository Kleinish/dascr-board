...
// FinishGame will handle finishing a game and saving final statistics
func FinishGame(h *ws.Hub) http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                uid := chi.URLParam(r, "id")
                if data, ok := Games[uid]; ok {
                        // Set end time
                        status := data.GameObject.GetStatus()
